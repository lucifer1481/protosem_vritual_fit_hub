...
while True:
    ...
    if result.pose_landmarks:
        ...
        # Width = shoulder width × 1.5, Height = neck to hip × 1.6
        shoulder_width = np.linalg.norm(r_shoulder - l_shoulder)
        torso_height = np.linalg.norm(mid_hip - neck)
        shirt_width = int(shoulder_width * 1.5)
        shirt_height = int(torso_height * 1.6)

        # Shirt position anchored at neck
        top_left_x = neck[0] - shirt_width // 2
        top_left_y = neck[1] - int(0.1 * shirt_height)

        # Choose shirt image based on orientation
